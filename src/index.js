...
class VersionHash {
   ...
   */
  dependencies() {
    return ["jsonfile", "escape-string-regexp", "path", "proxy-method"];
  }
  ...
  webpackConfig(webpackConfig) {
    ...
    let usesExtract =
      webpackConfig.optimization && webpackConfig.optimization.runtimeChunk;
    webpackConfig.output.filename = chunkhash;

    if (
      typeof webpackConfig.output.chunkFilename != "function" &&
      !usesExtract
    ) {
    ...
    forIn(webpackConfig.plugins, (value) => {
      if (
        value instanceof MiniCssExtractPlugin &&
        !value.options.filename.includes(contenthash)
      ) {
        let csspath = value.options.filename.substring(
          0,
          value.options.filename.lastIndexOf(".")
        );
            ...
            loader.options.name = (path) => {
              if (!/node_modules|bower_components/.test(path)) {
                return (
                  ...
                  `/[name]${delimiter}[hash:${length}].[ext]`
                );
              }
  ...
  registerHashAssets() {
    Mix.listen("build", () => {
      ...
      const removeHashFromKeyRegex = new RegExp(
        `${delimiter}([a-f0-9]{${op_length}})\\.([^.]+)$`,
        "g"
      );
      const removeHashFromKeyRegexWithMap = new RegExp(
        `${delimiter}([a-f0-9]{${op_length}})\\.([^.]+)\\.map$`,
        "g"
      );
      ...
      file.write(collect(newJson).sortKeys().all());
    });
    ...
    forIn(Mix.tasks, (task) => {
      if (task instanceof ConcatenateFilesTask) {
        proxyMethod.after(task, "merge", () => {
          ...
          const hash = `${this.getDelimiter()}${file
            .version()
            .substr(0, this.options.length)}`;
          const hashed = file.rename(
            `${file.nameWithoutExtension()}${hash}${file.extension()}`
          );
